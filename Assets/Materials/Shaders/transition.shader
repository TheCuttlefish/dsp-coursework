...
/////// Diffuse:
                ...
                float2 node_906 = (i.uv0*exp(_scale));
                float4 _Noise_var = tex2D(_Noise,TRANSFORM_TEX(node_906, _Noise));
                float node_8792 = floor(_steps);
                float3 node_3777 = saturate(floor((_Noise_var.rgb+(i.vertexColor.r*-2.0+1.0)) * node_8792) / (node_8792 - 1));
