    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float2 resolution = (float2(i.uv0.r,i.uv0.g)*_res*i.posWorld.a);
                float2 node_6907 = resolution;
                float4 _white_noise_var = tex2D(_white_noise,TRANSFORM_TEX(node_6907, _white_noise));
                float node_1806 = saturate(pow((distance((i.posWorld.rgb-objPos.rgb),((objPos.rgb-(_WorldSpaceCameraPos+i.normalDir))*(-1.0)))*_size),_power));
